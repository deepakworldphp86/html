...
abstract class Abstractmenu extends \Magento\Framework\View\Element\Template implements \Magento\Framework\DataObject\IdentityInterface
{
	...
	public function getCategoryMenu($item) {
        ...
        $html.=$item->getTitle();
        if (count($subCatAccepp) > 0) {
            $html.= ' <span class="icon-next hidden-xs hidden-sm"><i class="fa fa-caret-down"></i></span>';
        }
        $html.= '</a>';

        if (count($subCatAccepp) > 0 || $item->getTopContent() != '' || $item->getBottomContent() != '') {
            $html.='<span class="toggle-menu visible-xs-block visible-sm-block"><a onclick="toggleEl(this,\'mobile-menu-' . $item->getId() . '-'. $item->getParentId() . '\')" href="javascript:void(0)" class=""><em class="fa fa-plus"></em></a></span>';
	...
	public function drawList($category, $item, $level = 1) {
        ...
        if ($level < $maxLevel) {
            ...
            if (!empty($htmlChildren)) {
                $html[] = '<span class="toggle-menu visible-xs-block visible-sm-block"><a onclick="toggleEl(this,\'mobile-menu-cat-' . $category->getId() . '-' . $item->getParentId() . '\')" href="javascript:void(0)" class=""><em class="fa fa-plus"></em></a></span>';
